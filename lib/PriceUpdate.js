...
setInterval(async ()=> {
    ...
    async function indexUpdater() {
        const db = await pool.getConnection();
        try {
            const [indexPrice] = await db.query('select price from futures_inform')
            const newIndexPrice = (indexPrice[0].price + ((afterPriceSum - beforePriceSum) / 1000)).toFixed(2);
            await db.query('update futures_inform set price = ?',[newIndexPrice]);
            await db.query('insert into futures_pricelog values(?, ?, now())',[1, newIndexPrice]);
            beforePriceSum = 0;
            afterPriceSum = 0;
        } catch (err) {
            logWithTime(err);
