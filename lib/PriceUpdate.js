...
setInterval(async ()=> {
    ...
    async function IndexUpdater() {
        const db = await pool.getConnection();
        try {
            const [index_price] = await db.query('select price from index_inform')
            const New_index_price = (index_price[0].price + ((AfterPriceSum - BeforePriceSum) / 1000)).toFixed(2);
            await db.query('update index_inform set price = ?',[New_index_price]);
            await db.query('insert into index_pricelog values(?, ?, now())',[1, New_index_price]);
            BeforePriceSum = 0;
            AfterPriceSum = 0;
        } catch (err) {
            logWithTime(err);
