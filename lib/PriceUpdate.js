...
*/
setInterval(async ()=> {
    let BeforePriceSum = 0, AfterPriceSum = 0;
    ...
    async function StockPriceUpdater() {
        ...
        try {
            ...
            for (let i = stock_count[0].stock_id; i < stock_count[0].stock_id + stock_count.length; i++) {
                ...
                BeforePriceSum += results[0].price;
                ...
                await db.query(`insert into stock_pricelog (stock_id, price, log_at) values(${i}, ${valueChange}, now())`);
                AfterPriceSum += valueChange;
            }
        } catch (err) {
            logWithTime(err);
        } finally {
    ...
    async function IndexUpdater() {
        const db = await pool.getConnection();
        try {
            const [index_price] = await db.query('select price from futures_inform')
            const New_index_price = (index_price[0].price + ((AfterPriceSum - BeforePriceSum) / 1000)).toFixed(2);
            await db.query('update futures_inform set price = ?',[New_index_price]);
            await db.query('insert into futures_pricelog values(?, ?, now())',[1, New_index_price]);
            BeforePriceSum = 0;
            AfterPriceSum = 0;
        } catch (err) {
            logWithTime(err);
